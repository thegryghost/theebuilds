...
LIBRETRO_COMMIT_SHA="libretro-v2.8.0.3"
...
MY_PV="libretro-v${PV}"
#https://github.com/libretro/scummvm/archive/refs/tags/${P}.tar.gz
SRC_URI="https://github.com/libretro/scummvm/archive/refs/tags/${MY_PV}.tar.gz -> ${P}.tar.gz
	...
	https://github.com/libretro/libretro-deps/archive/${LIBRETRO_DEPS_SHA}.tar.gz -> ${PN}-libretro-deps-${LIBRETRO_DEPS_SHA:0:7}.tar.gz
        https://github.com/scummvm/scummvm/archive/refs/tags/v${SCUMMVM_VER}.tar.gz -> ${PN}-scummvm-${SCUMMVM_VER}.tar.gz
"
...
src_prepare() {
	...
	mv "${WORKDIR}/scummvm-${SCUMMVM_VER}" "${S}/scummvm-${SCUMMVM_VER}" || die
	eapply_user
}
