...
GLSLANG_SHA="1e4955adbcd9b3f5eaf2129e918ca057baed6520"
LINKERNSBYPASS_SHA="2f8df932534999196751dde3e0302f83ef1f4513"
CATCH2_SHA="3f0283de7a9c43200033da996ff9093be3ac84dc"
CUBEB_SHA="799e775484b8fce7e986ee7a4f4b651fec2bca07"
CRYPTOPP_SHA="af7d1050bf2287072edd629be133da458a3cf978"
DDS_KTX_SHA="42dd8aa6ded90b1ec06091522774feff51e83fc5"
OPENAL_SOFT_SHA="d9fed51aa6391debc31dbbca550f055c980afe70"
FMT_SHA="2dd4fa8742fdac36468f8d8ea3e06e78215551f8"
VMA_SHA="0e89587db3ebee4d463f191bd296374c5fafc8ea"
LODEPNG_SHA="18964554bc769255401942e0e6dfd09f2fab2093"
LIBRARY_HEADERS_SHA="3b3e28dbe6d033395ce2967fa8030825e7b89de7"
GOOGLETEST_SHA="40412d85124f7c6f3d88454583c4633e5e10fc8c"
...
OAKNUT_SHA="9d091109deb445bc6e9289c6195a282b7c993d49"
SDL2_SHA="ac13ca9ab691e13e8eebe9684740ddcb0d716203"
NIHSTRO_SHA="fd69de1a1b960ec296cc67d32257b0f9e2d89ac6"
FAAD2_SHA="09b3c850c606e7fedd06597223e54344e8d23c8c"
CRYPTOPP_CMAKE_SHA="9327192b0095dc1f420b2082d37bd427b5750d48"
SIRIT_SHA="4ab79a8c023aa63caaa93848b09b9fe8b183b1a9"
ENET_SHA="4f8e9bdc4ce6d1f61a6274b0e557065a38190952"
BOOST_SHA="3c27c785ad0f8a742af02e620dc225673f3a12d8"
XBYAK_SHA="a1ac3750f9a639b5a6c6d6c7da4259b8d6790989"
VULKAN_HEADERS_SHA="217e93c664ec6704ec2d8c36fa116c1a4a1e2d40"
CPP_JWT_SHA="e12ef06218596b52d9b5d6e1639484866a8e7067"
LIBRESSL_SHA="d4fc7348a3fbe9c659a373e28a3b50f052f7c50a"
...
DYNARMIC_SHA="ca0e264f4f962e29baa23a3282ce484625866b98"
DISCORD_RPC_SHA="20cc99aeffa08a4834f156b6ab49ed68618cf94a"
LIBADRENOTOOLS_SHA="deec5f75ee1a8ccbe32c8780b1d17284fc87b0f1"
...
SRC_URI="https://github.com/citra-emu/citra-nightly/archive/refs/tags/nightly-${PV}.tar.gz -> ${P}.tar.gz
	...
	https://github.com/GPUOpen-LibrariesAndSDKs/VulkanMemoryAllocator/archive/${VMA_SHA}.tar.gz -> ${PN}-vma-${VMA_SHA:0:7}.tar.gz
	https://github.com/lvandeve/lodepng/archive/${LODEPNG_SHA}.tar.gz -> ${PN}-lodepng-${LODEPNG_SHA:0:7}.tar.gz
	https://github.com/citra-emu/ext-library-headers/archive/${LIBRARY_HEADERS_SHA}.tar.gz -> ${PN}-library-headers-${LIBRARY_HEADERS_SHA:0:7}.tar.gz
	...
	https://github.com/knik0/faad2/archive/${FAAD2_SHA}.tar.gz -> ${PN}-faad2-${FAAD2_SHA:0:7}.tar.gz
...
src_prepare() {
	...
	rmdir "${S}/externals/glslang"
	if [ ! -e "${WORKDIR}/glslang-${GLSLANG_SHA}" ]; then
		mv "${WORKDIR}/glslang" "${S}/externals/glslang" || die
	else
		mv "${WORKDIR}/glslang-${GLSLANG_SHA}" "${S}/externals/glslang" || die
	fi
	...
	rmdir "${S}/externals/vma"
	if [ ! -e "${WORKDIR}/VulkanMemoryAllocator-${VMA_SHA}" ]; then
		mv "${WORKDIR}/VulkanMemoryAllocator" "${S}/externals/vma" || die
	else
		mv "${WORKDIR}/VulkanMemoryAllocator-${VMA_SHA}" "${S}/externals/vma" || die
	fi
	...
	rmdir "${S}/externals/library-headers"
	if [ ! -e "${WORKDIR}/ext-library-headers-${LIBRARY_HEADERS_SHA}" ]; then
		mv "${WORKDIR}/ext-library-headers" "${S}/externals/library-headers" || die
	else
		mv "${WORKDIR}/ext-library-headers-${LIBRARY_HEADERS_SHA}" "${S}/externals/library-headers" || die
	fi
	...
	rmdir "${S}/externals/faad2/faad2"
	if [ ! -e "${WORKDIR}/faad2-${FAAD2_SHA}" ]; then
		mv "${WORKDIR}/faad2" "${S}/externals/faad2/faad2" || die
	else
		mv "${WORKDIR}/faad2-${FAAD2_SHA}" "${S}/externals/faad2/faad2" || die
	fi
	...
	if [ ! -e "${WORKDIR}/-${LINKERNSBYPASS_SHA}" ]; then
		mv "${WORKDIR}/" "${S}/externals/libadrenotools/lib/linkernsbypass" || die
	else
		mv "${WORKDIR}/-${LINKERNSBYPASS_SHA}" "${S}/externals/libadrenotools/lib/linkernsbypass" || die
	fi
