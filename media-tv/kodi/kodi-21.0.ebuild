...
CODENAME="Omega"
FFMPEG_KODI_VERSION="Beta2"


GROOVY_VER="4.0.16"
COMMONS_TEXT_VER="1.11.0"
COMMONS_LANG_VER="3.14.0"
...
SRC_URI="
	...
	http://mirrors.kodi.tv/build-deps/sources/apache-groovy-binary-${GROOVY_VER}.zip
	https://dlcdn.apache.org//commons/text/binaries/commons-text-${COMMONS_TEXT_VER}-bin.tar.gz
	https://dlcdn.apache.org//commons/lang/binaries/commons-lang3-${COMMONS_LANG_VER}-bin.tar.gz
"
if [[ ${PV} == *9999 ]] ; then
...
src_unpack() {
	if [[ ${PV} == *9999 ]] ; then
		...
		unpack ${MY_P}.tar.gz
		unpack apache-groovy-binary-${GROOVY_VER}.zip
		unpack commons-text-${COMMONS_TEXT_VER}-bin.tar.gz
		unpack commons-lang3-${COMMONS_LANG_VER}-bin.tar.gz
		cp ${WORKDIR}/commons-*/*.jar "${WORKDIR}/groovy-${GROOVY_VER}/lib"
	fi
}
...
src_configure() {
	local mycmakeargs=(
		...
		-Dgroovy_SOURCE_DIR="${WORKDIR}/groovy-${GROOVY_VER}"
	)
