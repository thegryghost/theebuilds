...
src_prepare() {
	...
	# Use the correct defines to make gtkglext build work			# FIXME: Defined in glxext.h
	epatch "${FILESDIR}"/256.25-NVIDIA_glx-defines.patch
	# Use some more sensible gl headers and make way for new glext.h	# FIXME: Already done at the bottom of the file
	epatch "${FILESDIR}"/256.25-NVIDIA_glx-glheader.patch
	...
	if use kernel_linux; then
		...
		use custom-cflags && sed -i "s:-O:${CFLAGS}:" "${NV_SRC}"/Makefile.nvidia
